...
std::tuple<VectorXd, double, long long, bool> basis_reduction(const MatrixXd &B, double R, double sigma, long long num_samples, int seed)
{
	int n = B.rows();
	// int m = B.cols();
	...
	std::set<std::pair<double, vectorXd> > basis; // pair<norm, vector>
...
// Pybind11 bindings
PYBIND11_MODULE(basis_reduction, m)
{
	m.doc() = "Lattice basis reduction module using Pybind11";
